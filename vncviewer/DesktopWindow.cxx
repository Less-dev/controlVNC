...
DesktopWindow::DesktopWindow(int w, int h, const char *name, const rfb::PixelFormat& serverPF, CConn* cc_)
  : Fl_Window(w, h), cc(cc_), offscreen(nullptr), overlay(nullptr), firstUpdate(true),
    delayedFullscreen(false), delayedDesktopSize(false), keyboardGrabbed(false), mouseGrabbed(false),
    statsLastUpdates(0), statsLastPixels(0), statsLastPosition(0), statsGraph(nullptr)
{
    Fl_Group* group;
  ...
  //resizable(group);
    border(1);
    // Crear el viewport y las barras de desplazamiento dentro del grupo
    std::cout << "Ancho del Viewport: " << w << ", Alto: " << h << std::endl;
    ...
    group->end();  // Finaliza el grupo para evitar que otros widgets se añadan por error

    // Crear la barra lateral como una ventana separada (fuera del grupo)
    sidebar = new SideBar(x() + w - EDGE_SCROLL_SIZE - 10, y(), 60, h);
    sidebar->show();  // Asegurarse de mostrar la barra lateral
    
    // Callback para actualizar la posición de la barra lateral en función de la ventana principal
    this->callback([](Fl_Widget*, void* data) {
        DesktopWindow* win = static_cast<DesktopWindow*>(data);
        int scrollbar_width = EDGE_SCROLL_SIZE;  // Usar el tamaño definido de la barra de scroll
        win->sidebar->update_position(win->x() + win->w() - scrollbar_width - 60, win->y(), 60, win->h());
    }, this);
...
int sx, sy, sw, sh;  // Variables para la posición y tamaño de la pantalla ya declaradas aquí

// Aquí se usa la misma variable sin redeclararla
if (force_position()) {
  Fl::screen_work_area(sx, sy, sw, sh, geom_x, geom_y);
} else {
  int mx, my;

  // Si no se solicita explícitamente una posición, asumimos que el cursor sigue el comportamiento del monitor
  Fl::get_mouse(mx, my);
  Fl::screen_work_area(sx, sy, sw, sh);

  // Ajustar el tamaño de la ventana verticalmente al 90% de la altura de la pantalla
  h = static_cast<int>(sh * 0.9);  // 90% de la altura de la pantalla

  // Si también quieres ajustar el ancho (opcional), puedes hacer lo mismo para el ancho
  w = static_cast<int>(sw * 0.32);  // Por ejemplo, ajustar al 90% del ancho también

  // Configurar el tamaño de la ventana basado en estos valores
  size(w, h);  // Ajustar la ventana con el nuevo tamaño
...
void DesktopWindow::setName(const char *name)
{
  ...
  snprintf(windowNameStr, 256, "%.240s", name);
...
void DesktopWindow::draw()
{
  ...
  if (redraw) {
      fl_color(fl_rgb_color(138, 3, 3));
        if (offscreen) {
            offscreen->clear(138, 3, 3);  // Llenar con rojo sangre en el buffer
        } else {
            fl_rectf(0, 0, W, H);  // Dibujar el fondo rojo en la pantalla
        }
  }
  ...
  if (redraw) {
    ...
    fl_line_style(FL_SOLID, border_width);
